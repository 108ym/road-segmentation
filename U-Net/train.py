...
def train(model, trainloader, validloader, criterion, optimizer, epochs, device, load_pth, model_sv_pth, plot=True, visualize=False, load_model=False):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(trainloader):
            ...
            inputs, mask = inputs.float(), mask.float()  
...
if __name__ == "__main__":
    ...
    train_data = Dataset(img_pth = path + 'train/', mask_pth = path + 'train_labels/', transform = train_transforms)
    valid_data = Dataset(img_pth = path + 'val/', mask_pth = path + 'val_labels/', transform = valid_transforms)
